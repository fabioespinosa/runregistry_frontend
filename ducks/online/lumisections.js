...
export const fetchJointLumisectionRanges = (run_number, dataset_name) =>
  error_handler(async (dispatch) => {
    const {
      data: lumisections,
    } = await axios.post(`${api_url}/lumisections/joint_lumisection_ranges`, {
      run_number,
      dataset_name,
    });
    dispatch({
...
export const addLumisectionRange = (
  ...
  error_handler(async (dispatch, getState) => {
    const { data: lumisections } = await axios.put(
      `${api_url}/lumisections/edit_rr_lumisections`,
...
export const addOMSLumisectionRange = (
  new_lumisection_range,
  run_number,
  dataset_name,
  component
  ...
  error_handler(async (dispatch, getState) => {
    const { data: lumisections } = await axios.put(
      `${api_url}/lumisections/edit_oms_lumisections`,
      { new_lumisection_range, run_number, dataset_name, component },
      auth(getState)
    );
    dispatch({
