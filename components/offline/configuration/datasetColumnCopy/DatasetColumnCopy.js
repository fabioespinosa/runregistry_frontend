  ...
  render() {
    ...
    workspaces.forEach(
      ({ workspace: iterator_workspace, columns: iterator_columns }) => {
        if (workspace.toLowerCase() === iterator_workspace.toLowerCase()) {
    ...
    return (
      <div>
        <h3>
          This tool will allow the DC Expert to copy certain columns across
          datasets
        </h3>
        ...
        {workspace === 'global' ? (
          <h1>
            This action cannot be performed in GLOBAL workspace, it must be done
            individually on each workspace
          </h1>
        ) : (
          <div>
            <Formik
              ...
              onSubmit={async values => {
                ...
                await Swal(`Columns copied`, '', 'success');
              }}
              render={({ values, setFieldValue, handleSubmit }) => {
                ...
                return (
                  <form onSubmit={handleSubmit} className="dataset_copy_form">
                    <center>
                      This tool serves as a way to copy a column (say pixel from
                      tracker) from a specific dataset to the same column of
                      another dataset. <br /> The classical use case is if a
                      subsystem did a batch update of many datasets of a ReReco
                      or UltraLegacy (UL) and now want to revert the change they
                      made. <br />
                      So we use this tool so that they can copy that column they
                      changed back into what was the original PromptReco value
                    </center>
                    <div className="form_container">
                      <br />
                      Source dataset name (whose columns we want to copy from):
                      <Select
                      ...
                      <br />
                      <br />
                      Target dataset name (whose columns we want to copy to):
                      <Select
                        placeholder="Target dataset name"
                        value={values['target_dataset_name']}
                        onChange={value =>
                          setFieldValue('target_dataset_name', value)
                        }
                      >
                        {unique_dataset_names.map(dataset_name => (
                      ...
                      <br />
                      These columns all belong to the {workspace} workspace{' '}
                      <br />
                      <ul>
                        {columns.map(column => (
                          <li key={column}>
                            <Checkbox
                              checked={values[`columns*${column}`]}
                              onChange={({ target }) =>
                                setFieldValue(
                                  `columns*${column}`,
                                  target.checked
